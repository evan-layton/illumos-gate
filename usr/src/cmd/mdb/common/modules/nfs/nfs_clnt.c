...
nfs4_foo_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	mdb_printf("Address       Cred             RefCnt   SeqID    JustCre SeqInUse BadSeqid\n");
 ...
 */
int
nfs4_os_dcmd(uintptr_t addr, uint flags, int argc, const mdb_arg_t *argv)
{
	int opts = 0;

	if (mdb_getopts(argc, argv, 'v', MDB_OPT_SETBITS, TRUE, &opts, NULL) != argc) {
		return (DCMD_USAGE);
	}
