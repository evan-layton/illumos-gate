...
int
nfs_stat_dcmd(uintptr_t addr, uint flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_ADDRSPEC) {
		...
		if (mdb_readsym(&zonep, sizeof (uintptr_t), "global_zone") == -1) {
	...
	if (mdb_vread(&mdb_stats.nfsglbls, sizeof (struct nfs_globals), glbls) == -1) {
	...
	if (mdb_vread(&mdb_stats.nfsstats, sizeof (struct nfs_stats), glbls) == -1) {
		mdb_warn("Failed to read nfs_stats at %p", glbls);
		return (DCMD_ERR);
	}
	...
	if (mdb_vread(&mdb_stats.rpcstats, sizeof (struct rpcstat), glbls) == -1) {
		mdb_warn("Failed to read nfs_stats at %p", glbls);
		return (DCMD_ERR);
	}
	...
	if (zoned_get_zsd(zonep, "nfs4_callback_zone_key", (uintptr_t *)&cb_glbls)) {
		mdb_warn("Failed to find %s", "nfs4_callback_zone_key");
		return (DCMD_ERR);
	}

	mdb_stats.callback_stats =
	    (cb_glbls + offsetof(struct nfs4_callback_globals, nfs4_callback_stats));
...
nfs_srvstat(nfs_mdb_stats_t *stptr, int flag)
{
	...
	if (flag & NFS_V2_STAT) {
		mdb_printf("NFSv2\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsglbls.svstat[2], "svstat_tmpl") ||
	...
	if (flag & NFS_V3_STAT) {
		mdb_printf("NFSv3\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsglbls.svstat[3], "svstat_tmpl") ||
	...
	if (flag & NFS_V4_STAT) {
		mdb_printf("NFSv4\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsglbls.svstat[4], "svstat_tmpl") ||
...
nfs_clntstat(nfs_mdb_stats_t *stptr, int flags)
{
	...
	if (flags & NFS_V2_STAT) {
		...
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v2.rfsreqcnt_ptr,
		    "rfsreqcnt_v2_tmpl"))
			return (-1);
	}
	if (flags & NFS_V3_STAT) {
		mdb_printf("Version 3\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v3.rfsreqcnt_ptr,
		    "rfsreqcnt_v2_tmpl"))
			return (-1);
	}
	if (flags & NFS_V4_STAT) {
		...
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v4.rfsreqcnt_ptr,
		    "rfsreqcnt_v2_tmpl"))
			return (-1);
	}
	return (0);
...
nfs_clntstat_acl(nfs_mdb_stats_t *stptr, int flags)
{
	...
	if (flags & NFS_V2_STAT) {
		mdb_printf("Version 2\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v2.aclreqcnt_ptr,
		    "aclreqcnt_v2_tmpl"))
			return (-1);
	}
	if (flags & NFS_V3_STAT) {
		mdb_printf("Version 3\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v3.aclreqcnt_ptr,
		    "aclreqcnt_v3_tmpl"))
			return (-1);
	}
	if (flags & NFS_V4_STAT) {
		mdb_printf("Version 4\n");
		if (prt_nfs_stats((uintptr_t)stptr->nfsstats.nfs_stats_v4.aclreqcnt_ptr,
		    "aclreqcnt_v4_tmpl"))
			return (-1);
	}
	return (0);
...
prt_nfs_stats(uintptr_t addr, char *name)
{
	...
	for (i = 0; i < count; i++) {
		if (mdb_vread(&kstats, sizeof (kstat_named_t), addr + (i * sizeof (kstat_named_t))) < 0) {
			status = 1;
			goto done;
		}
		mdb_snprintf(&kstat_line[NFS_STAT_NUM_CLMNS * i], NFS_STAT_NUM_CLMNS, "%s", kstats.name);
...
kstat_prtout(char *ks_line, uint64_t *values, int count)
{
	...
	while (i < count) {
		mdb_printf("%-*s", NFS_STAT_NUM_CLMNS, &ks_line[NFS_STAT_NUM_CLMNS * i]);

		num++;
		if (num == NFS_STAT_NUM_STATS / NFS_STAT_NUM_CLMNS) {
			mdb_printf("\n");
			while (num > 0) {
				mdb_snprintf(val_str, 24, "%ld  ", values[i+1-num]);
