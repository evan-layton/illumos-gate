...
nfs_exptable_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (mdb_pwalk_dcmd("nfs_expinfo", "nfs_expinfo", 0, NULL, (uintptr_t)nfsglbls.nfs_export->exptable) == -1) {
...
nfs_nstree_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (mdb_vread(&exp, sizeof (nfs_export_t), (uintptr_t)nfsglbls.nfs_export) == -1) {
